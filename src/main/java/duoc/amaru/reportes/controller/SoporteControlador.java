...
public class SoporteControlador {
    ...
    public ResponseEntity<?> postSoporte(@RequestBody CrearSoliDTO soli, @PathVariable Long userId) {
        Solicitud reply = solicitudServicio.crearSolicitud(soli, userId, false);
    ...
    public ResponseEntity<?> getHistorial(@PathVariable Long id) {
        List<Solicitud> reply = solicitudServicio.verHistorial(id, false);
        if (reply.isEmpty())
            return ResponseEntity.status(404).body("No tienes solicitudes de soporte registradas");

        return ResponseEntity.status(404).body(reply);
    }

    // FILTRAR HISTORIAL POR ESTADO DE LA SOLICITUD DE SOPORTE
    @GetMapping("/cliente:{id}/{estado}")
    public ResponseEntity<?> getHistorialPorEstado(@PathVariable Long id, @PathVariable String estado) {
        List<Solicitud> reply = solicitudServicio.findHistorialByEstado(id, false, estado);
        if (reply.isEmpty())
    ...
    // OBTENER SOLICITUD SOPORTE POR ID
    @GetMapping("/usuario:{userId}/soporte:{soliId}")
    ...
    // MARCAR SOLICITUD DE SOPORTE COMO REVISADA
    @PutMapping("empleado:{userId}/en-revision/{soliId}")
    public ResponseEntity<?> putRevisarSoporte(@PathVariable Long userId, @PathVariable Long soliId) {
    ...
    // MARCAR SOLICITUD DE SOPORTE COMO RESUELTA
    @PutMapping("empleado:{userId}/resolver/{soliId}")
    public ResponseEntity<?> putResolverSoporte(@PathVariable Long userId, @PathVariable Long soliId) {
    ...
    // MARCAR SOLICITUD DE SOPORTE COMO CERRADO
    @PutMapping("empleado:{userId}/cerrar/{soliId}")
    public ResponseEntity<?> putCerrarSoporte(@PathVariable Long userId, @PathVariable Long soliId) {
